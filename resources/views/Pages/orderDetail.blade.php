                        ...
                        <div class="col l-12 m-12 c-12">
                            <div class="main" id="main">
                                <div class="form" id="form-infor">
                                    <h3 class="heading"><strong>Mã đơn hàng {{ $donHang->id }}</strong></h3>
                                        ...
                                        <div class="table_page table-responsive">
                                            <table>
                                                ...
                                                <tr>
                                                    ...
                                                    <th class="product_quantity">Ngày nhận hàng <br>
                                                    </th>
                                                    <th class="product_quantity">Giá</th>
                                                    <th class="product_quantity">Số lượng</th>
                                                    <th class="product_quantity">Tổng tiền 
                                                        <br> 
                                                        Tiền trả: {{ number_format($donHang->tongTien, 0, ',', '.') }}<sup>₫</sup><br>
                                                        @if (isset($donHang->giamGia))
                                                            Giảm: {{ number_format($donHang->giamGia, 0, ',', '.') }}<sup>₫</sup><br>
                                                        @endif
                                                        @if (isset($donHang->ghiChu))
                                                        {{ $donHang->ghiChu }} <br>
                                                        @endif
                                                        @if ($donHang->daThanhToan)
                                                            <strong style="color: rgb(33, 90, 233)">(Đã thanh toán)</strong>
                                                ...
                                                <tbody id="cartUpdate">
                                                    @foreach ($chiTietDonHangs as $item)
                                                        @php
                                                            $dungCu = App\Models\DungCu::where('maDungCu',$item->maDungCu)->first();
                                                        @endphp
                                                        <tr id="">
                                                            <td class="product_thumb">
                                                                <a><img src="https://minhloiiot.id.vn/assets/img/{{$dungCu->hinhAnh1}}" style="width: 100px; height: 66px; object-fit: cover;"></a>
                                                            </td>
                                                            ...
                                                            <td class="product-date">{{ \DateTime::createFromFormat('Y-m-d H:i:s', $donHang->ngayDatHang)->format('d-m-Y H:i:s') }}</td>
                                                            <td>
                                                                @if (isset($donHang->ngayNhanHang))
                                                                    {{ \DateTime::createFromFormat('Y-m-d H:i:s', $donHang->ngayNhanHang)->format('d-m-Y H:i:s') }}
                                                                @else
                                                                    <strong style="color: red">Chưa nhận</strong><br>
                                                                @endif
                                                            </td>
                                                            <td class="product_total">
                                                                {{ number_format($dungCu->donGiaGoc, 0, ',', '.') }} <sup>₫</sup>
                                                            </td>
                                                            <td>
                                                                {{ $item->soLuong }}
                                                            </td>
                                                    ...
                                                    @endforeach
                                                    {{-- @if(isset($chiTietDonHangs->maDungCu))
                                                        <tr>
                                                            @php
                                                                $tool = App\Models\DungCu::where('maDungCu',$chiTietDonHangs->maDungCu)->first();
                                                                ...
                                                                echo '<td>Thuê dụng cụ<br/>'.number_format($chiTietDonHangs->gia, 0, ',', '.').'<sup>₫</sup></td>';
                                                                echo '<td>Số lượng: '.$chiTietDonHangs->soLuong.'</td>';
                                                            @endphp
                                                        </tr>
                                                    @endif --}}
                                                </tbody>
